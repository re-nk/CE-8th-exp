...
variaveis_independentes_e_seus_niveis = {
    "density": np.arange(0.05,0.75,0.1), # 5x
    "minority_pc": np.arange(0.1,0.6,0.1), # 5X
    "homophily": np.arange(1,5,1), # 4X 
    "polarization": np.arange(0.1,1.1,0.1)
}
...
replicacoes=1 # no desenho experimental, esse parâmetro é chamado de replicação, e indica a quantidade de replicações de cada tratamento 
...
qtd_maxima_passos_para_estabilizar = 1 # qtd de interações necessárias para o fenômeno é estabilizar
